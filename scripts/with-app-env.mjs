...
/**
 * The command to actually spawn, as `[command, args]`.
 *
 * On Windows a bare `vite` resolves to `node_modules/.bin/vite.CMD`, which
 * spawn() cannot exec without a shell — and a shell concatenates args
 * unescaped (DEP0190). Running the package's own bin script under this node
 * instead needs neither, and a path that already exists (`process.execPath`)
 * or any POSIX command is passed straight through unchanged.
 */
export function resolveCommand(command, args, platform = process.platform) {
  ...
  try {
    ...
    // Not a resolvable package (a real PATH binary) — let spawn report it.
    return [command, args];
  }
}
...
function main(argv) {
  ...
  const [cmd, cmdArgs] = resolveCommand(command, args);
