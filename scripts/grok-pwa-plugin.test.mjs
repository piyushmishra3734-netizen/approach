...
// Calls that omit `cwd`/`site` are testing the no-workspace defaults, so they
// must not read this app's real src/lib/og/site.json or public/og.jpg — with
// the repo as cwd every such assertion picks up the shipped app identity
// instead. Tests that want the workspace use the absolute TEMPLATE_ROOT.
...
test("a cwd card file upgrades an explicit site without card=custom", () => {
  // normalizeHeadContext runs applyCustomCardFromFs over the passed-in site on
  // purpose: a public/og.jpg written after the middleware baked its snapshot
  // must still beat the og.grok.me placeholder.
