...
test("the CLI still runs when invoked through a symlinked path", async () => {
  ...
  // "junction" is ignored on POSIX and is the one dir link Windows allows
  // without elevation, so this stays a real test on both platforms.
...
test("resolveCommand runs a local bin under node on Windows, untouched elsewhere", () => {
  assert.deepEqual(resolveCommand("vite", ["dev"], "linux"), ["vite", ["dev"]]);
  assert.deepEqual(resolveCommand(process.execPath, ["-e", "0"], "win32"), [
    process.execPath,
    ["-e", "0"],
  ]);
  const [cmd, args] = resolveCommand("vite", ["dev", "--port", "8080"], "win32");
  assert.equal(cmd, process.execPath);
  assert.equal(args[0], join(projectRoot(), "node_modules/vite/bin/vite.js"));
  assert.deepEqual(args.slice(1), ["dev", "--port", "8080"]);
  // An unresolvable name falls through so spawn reports the real ENOENT.
  assert.deepEqual(resolveCommand("definitely-not-a-package", [], "win32"), [
